...
import { fetchNews, fetchNewsByDistrict, fetchNewsByPanchayat } from '@/redux/slices/newsSlice';
import { fetchDistricts } from '@/redux/slices/districtSlice';
import { fetchPanchayats } from '@/redux/slices/panchayatSlice';
import { Calendar, ArrowRight, Search, Newspaper, ChevronDown, Loader2, Filter } from 'lucide-react';
...
export default function NewsPage() {
  ...
  const { news, loading, error, lastFetched, totalNews, currentPage: reduxPage, totalPages } = useSelector((state) => state.news);
  ...
  // Click outside handler - Fixed to not close when clicking dropdown items
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      // Check if click is outside district dropdown
      if (districtRef.current && !districtRef.current.contains(event.target)) {
        setIsDistrictOpen(false);
      }
      // Check if click is outside panchayat dropdown
      if (panchayatRef.current && !panchayatRef.current.contains(event.target)) {
        setIsPanchayatOpen(false);
      }
      // Check if click is outside filters container on mobile
  ...
  useEffect(() => {
    ...
    const panchayatParam = searchParams.get('panchayat');
    const categoryParam = searchParams.get('category');
    
    if (districtParam && districtParam !== 'all') {
      setSelectedDistrict(districtParam);
    }
    if (panchayatParam && panchayatParam !== 'all') {
      setSelectedPanchayat(panchayatParam);
    }
  ...
  }, [dispatch, districts.length, panchayats.length]);
  ...
  useEffect(() => {
    const fetchNewsData = () => {
      ...
      // Priority: Panchayat > District > All
      if (selectedPanchayat !== 'all') {
        dispatch(fetchNewsByPanchayat({ panchayatId: selectedPanchayat, params }));
      } else if (selectedDistrict !== 'all') {
        dispatch(fetchNewsByDistrict({ districtId: selectedDistrict, params }));
      } else {
    ...
    fetchNewsData();
  }, [dispatch, selectedCategory, selectedDistrict, selectedPanchayat, currentPage]);
  ...
  // Panchayat options (filtered by district)
  ...
  // Filtered news (client-side search filter)
  ...
  const handleDistrictChange = useCallback((district) => {
    ...
    setSelectedPanchayat('all'); // Reset panchayat when district changes
    setIsDistrictOpen(false);
  }, []);

  const handlePanchayatChange = useCallback((panchayat) => {
                  ...
                  <div className="grid grid-cols-1 gap-3 pt-3 border-t border-[#117307]/20">
                    ...
                    <div className="relative" ref={categoryRef}>
                      ...
                      >
                        ...
                        <ChevronDown size={20} className={`flex-shrink-0 transform transition-transform ${isCategoryOpen ? 'rotate-180' : ''}`} />
                      </button>
                    ...
                    <div className="relative" ref={districtRef}>
                      ...
                      >
                        ...
                        <ChevronDown size={20} className={`flex-shrink-0 transform transition-transform ${isDistrictOpen ? 'rotate-180' : ''}`} />
                      </button>
                    ...
                    <div className="relative" ref={panchayatRef}>
                      ...
                      >
                        ...
                        <ChevronDown size={20} className={`flex-shrink-0 transform transition-transform ${isPanchayatOpen ? 'rotate-180' : ''}`} />
                      </button>
                      ...
                      {isPanchayatOpen && (
                        <div className="absolute z-30 w-full mt-1 bg-white border-2 border-[#117307] rounded-lg shadow-lg overflow-hidden max-h-60 overflow-y-auto">
                          {panchayatOptions.map((option) => (
                            <button
                              key={option.value}
                              onClick={() => handlePanchayatChange(option.value)}
                              className={`w-full text-left px-4 py-3 hover:bg-[#f5fbf2] font-semibold ${
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        {loading && news.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[#117307] text-lg font-medium">Loading news...</p>
          </div>
        ) : error ? (
          <div className="text-center py-16 bg-white rounded-2xl shadow-sm">
            <Newspaper size={48} className="mx-auto text-red-500 mb-4" />
            <p className="text-[#117307] text-lg font-medium">Failed to load news</p>
          </div>
        ) : filteredNews.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl shadow-sm">
            <Newspaper size={48} className="mx-auto text-[#117307] opacity-20 mb-4" />
            <p className="text-[#117307] text-lg font-medium mb-2">No news found</p>
              ...
              {filteredNews.map((item) => (
                ...
                >
                  <div className="h-52 bg-[#f5fbf2] relative overflow-hidden">
                    ...
                    <div className="fallback-icon absolute inset-0 flex items-center justify-center bg-[#117307]/5" style={{ display: item.featuredImage ? 'none' : 'flex' }}>
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-2 text-[#4d674f] font-medium text-sm mb-3">
                      ...
                      <span>
                        {new Date(item.createdAt || item.publishDate).toLocaleDateString('en-US', { 
...
// import { fetchDistricts } from '@/redux/slices/districtSlice';
// import { fetchPanchayats } from '@/redux/slices/panchayatSlice';
// import { Calendar, ArrowRight, Search, Newspaper, ChevronDown, Loader2 } from 'lucide-react';
// import TextField from '@/components/ui/TextField';
...
//                     <button
//                       onClick={() => setIsCategoryOpen(!isCategoryOpen)}
//                       className="w-full flex items-center justify-between px-4 py-3 border-2 border-[#117307] rounded-lg text-[#117307] font-semibold bg-white hover:bg-[#f5fbf2] text-sm"
//                     >
//                       <span className="truncate">{currentCategoryLabel}</span>
//                       <ChevronDown size={18} className={`flex-shrink-0 transform transition-transform ${isCategoryOpen ? 'rotate-180' : ''}`} />
//                     </button>
...
//                             key={option.value}
//                             onClick={() => handleCategoryChange(option.value)}
//                             className={`w-full text-left px-4 py-3 hover:bg-[#f5fbf2] font-semibold text-sm ${
//                               selectedCategory === option.value ? 'bg-[#117307] text-white' : 'text-[#117307]'
//                             }`}
//                           >
//                             {option.label}
...
//                     >
//                       <span className="truncate">{currentDistrictLabel}</span>
//                       <ChevronDown size={18} className={`flex-shrink-0 transform transition-transform ${isDistrictOpen ? 'rotate-180' : ''}`} />
//                     </button>
...
//               {/* <div className="mt-4 pt-4 border-t border-[#117307]/10 text-center">
