...
export async function GET(request) {
    try {
        ...
        const recentPanchayats = await GramPanchayat.find()
            .select('name slug district headerImage createdAt')
            .populate('district', 'name')
            .sort({ createdAt: -1 })
            .limit(5);

        // UPDATED: Include thumbnailUrl in the media query
        const recentMedia = await Media.find({ status: 'approved' })
            .select('title fileType thumbnailUrl fileUrl createdAt') // Added thumbnailUrl here
            .sort({ createdAt: -1 })
            .limit(5);
