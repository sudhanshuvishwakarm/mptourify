    ...
    try {
        ...
        if (!currentAdmin || currentAdmin.role !== 'admin') {
            return NextResponse.json(
                { success: false, message: "Unauthorized. Only admins can change panchayat status." },
                { status: 403 }
            );
        }
        ...
        // Updated to match new model enum
        if (!status || !['Verified', 'Pending', 'Draft'].includes(status)) {
            return NextResponse.json(
                { success: false, message: "Invalid status. Must be 'Verified', 'Pending', or 'Draft'" },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { success: true, message: `Panchayat status changed to '${status}' successfully`, panchayat },
            { status: 200 }
        );
