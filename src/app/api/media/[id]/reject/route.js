    ...
    try {
        ...
        if (!currentAdmin || currentAdmin.role !== 'admin') {
            return NextResponse.json(
                { 
                    success: false,
                    message: "Unauthorized. Only admins can reject media." 
                },
                { status: 403 }
            );
        }
        ...
        if (media.status === 'rejected') {
            return NextResponse.json(
                { 
                    success: false,
                    message: "Media is already rejected" 
                },
                { status: 400 }
            );
        }
        ...
        if (reason) {
            // Store rejection reason in description (or you can add a rejectionReason field to model)
            media.description = `[REJECTED: ${reason}] ${media.description}`;
        }
        await media.save();
