...
export default function AllPanchayatsPage() {
  ...
  useEffect(() => {
    const hasCachedData = panchayats.length > 0;
    
    if (!hasCachedData) {
      dispatch(fetchPanchayats({ limit: 100 })).finally(() => {
  ...
  const handleCardClick = (id) => {
    router.push(`/admin/panchayats/${id}`);
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 2, md: 3 } }}>
      {isInitialLoading && panchayats.length === 0 && (
      ...
      <Card sx={{ border: '1px solid #144ae920' }}>
        {panchayats.length === 0 && !isInitialLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ">
            {filteredPanchayats.map((panchayat) => (
              <div
                key={panchayat._id}
                className={`bg-white rounded-lg border border-blue-100 shadow-sm hover:shadow-lg hover:border-blue-300 transition-all duration-200 ease-in-out overflow-hidden relative cursor-pointer ${
                  deletingId === panchayat._id ? 'opacity-60' : 'opacity-100'
                }`}
                onClick={() => handleCardClick(panchayat._id)}
              >
