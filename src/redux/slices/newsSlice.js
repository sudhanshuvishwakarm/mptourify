...
// FETCH NEWS BY PANCHAYAT
export const fetchNewsByPanchayat = createAsyncThunk(
    'news/fetchNewsByPanchayat',
    async ({ panchayatId, params = {} }, { rejectWithValue }) => {
        try {
            const queryString = new URLSearchParams(params).toString();
            const res = await axios.get(`/api/news/panchayat/${panchayatId}?${queryString}`);
            return res.data;
        } catch (error) {
            return rejectWithValue(error.response?.data || error.message);
        }
...
const newsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            // FETCH BY PANCHAYAT
            .addCase(fetchNewsByPanchayat.pending, (state) => {
                state.loading = true;
            })
            .addCase(fetchNewsByPanchayat.fulfilled, (state, action) => {
                state.loading = false;
                state.news = action.payload.news || [];
                state.totalNews = action.payload.totalNews || 0;
                state.currentPage = action.payload.currentPage || 1;
            ...
            .addCase(fetchNewsByPanchayat.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload?.message || 'Failed to fetch panchayat news';
            });
    }
});
